...
import { forgotPasswordOtp, resendOtp, verifyEmail } from "@/services/auth/auth.api"

export default function OtpVerificationPage() {
  const router = useRouter()
  const [otpDigits, setOtpDigits] = useState<string[]>(Array(8).fill(""))
  const [email, setEmail] = useState("")
  const [mode, setMode] = useState<"register" | "forgot" | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    if (router.query.mode === 'register' || router.query.mode === 'forgot') {
      setMode(router.query.mode as 'register' | 'forgot')
    }
    ...
    try {
      if (mode === 'register') {
        ...
        if (result.code === 200) {
          ...
          router.push(`/auth/forgot-password?email=${encodeURIComponent(email)}&step=3`)
        } else {
          toast.error(result.message || "Mã OTP không đúng")
        }
      }
        ...
        <Card className="w-full max-w-md bg-white shadow-xl">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="text-center space-y-2">
              ...
              {countdown > 0 && (
                <p className="text-xs text-gray-500">Mã OTP sẽ hết hạn sau {countdown} giây</p>
              )}
            </div>
